...
- **Entry point:** `src/index.ts` — registers all 19 tools with Zod schemas
...
slash-commands/         # .md files copied to ~/.claude/commands/ (11 commands, incl. devctx-linear)
```
...
- Linear integration: `devctx_linear_sync` tool syncs todos with Linear issues (requires `LINEAR_API_KEY` env var; config stored in `.devctx/linear.json`)
